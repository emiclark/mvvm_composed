//
//  ComposedCollectionViewCell.swift
//  TechChallenge
//
...
class ComposedCollectionViewCell: UICollectionViewCell  {
    //MARK:- Properties
    ...
    private func setupViews() {
        ...
        logoImageView.snp.makeConstraints { make in
            make.left.top.equalTo(contentView).offset(offset).multipliedBy(1.5)
            make.height.width.equalTo(imageHeight)
        }

        centerStackview.snp.makeConstraints { make in
            make.top.equalTo(contentView).offset(offset).multipliedBy(1.5)
